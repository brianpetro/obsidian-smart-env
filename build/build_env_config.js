...
export function build_smart_env_config(dist_dir, roots, version=pkg.version) {
  ...
  const collections_config = collections_loader.build_config({version});
  const items_config = items_loader.build_items_config({version});
  const modules_config = modules_loader.build_config({version});
  const components_config = components_loader.build_config({version});
  const actions_config = actions_loader.build_config({version});
