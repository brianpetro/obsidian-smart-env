...
export function build_smart_env_config(dist_dir, roots) {
  ...
  for (const root of roots) {
    ...
    // actions
    const { flat: a_flat, nested: a_nested } = scan_actions(root);
    a_flat.forEach(({ import_var, export_name, import_path }) => {
      all_actions_flat_map.set(import_var, { import_var, export_name, import_path });
    });
  ...
  const action_imports = all_actions_flat
    .map(({ export_name, import_var, import_path }) => `import { ${export_name} as ${import_var} } from '${import_path}';`)
    .join('\n');
  ...
  // actions
  const actions_config = components_to_string(all_actions_nested, 4);
...
`
   ...
   *   - flat array of {export_name, import_var, import_path} (for imports)
   *   - nested object describing final config tree
   *
   * Rules:
    ...
    function walk(curr_dir, rel_parts) {
      fs.readdirSync(curr_dir).forEach(entry => {
        ...
        const has_named_export =
          content.includes(`export function ${action_name}`) ||
          content.includes(`export async function ${action_name}`) ||
          content.includes(`export const ${action_name}`) ||
          content.includes(`export let ${action_name}`) ||
          content.includes(`export var ${action_name}`) ||
          new RegExp(`export\\s*\\{[^}]*\\b${action_name}\\b[^}]*\\}`).test(content);

        if (!has_named_export) return;
        ...
        // flat list
        flat.push({ export_name, import_var, import_path });
