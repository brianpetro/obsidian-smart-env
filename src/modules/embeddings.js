...
export class Embeddings {
  ...
  top_k(params = {}) {
    ...
    const file_info = this.get_active_file_info();
    const vectors = this._vectors_by_file?.[file_info?.file];
    ...
    const items = Object.values(this.collection?.items || {});
    for (let item_i = 0; item_i < items.length; item_i += 1) {
      const item = items[item_i];
      const ref = this.get_item_embedding_ref(
        item,
        undefined,
  ...
  /**
   * Calculate raw cosine similarity for stored items or transient { vec } targets.
   * Vector getters retain their storage/ref validation; each vector is read once.
   * Missing vectors throw coded errors so score actions can preserve their result
   * contract without forcing Pro to materialize healthy indexed vectors first.
   *
   * @param {object} from_item
   * @param {object} to_item
   * @returns {number}
   * @throws {Error} With code MISSING_FROM_VECTOR or MISSING_TO_VECTOR when absent.
   * @throws {Error} When vector dimensions differ; other calculation errors propagate.
   */
  cosine_similarity(from_item, to_item) {
    const from_vec = from_item?.vec;
    if (!from_vec) {
      const error = new Error('Missing from_item.vec');
      error.code = 'MISSING_FROM_VECTOR';
      throw error;
    }
    const to_vec = to_item?.vec;
    if (!to_vec) {
      const error = new Error('Missing to_item.vec');
      error.code = 'MISSING_TO_VECTOR';
      throw error;
    }
    return cos_sim(from_vec, to_vec);
  }
