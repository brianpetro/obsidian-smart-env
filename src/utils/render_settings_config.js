      ...
      switch (setting_config.type) {
        ...
        case 'toggle':
          setting.addToggle((toggle) => {
            toggle.setValue(get_setting_value() || false);
        ...
        case 'number':
          setting.addText((text) => {
            text.setValue(String(get_setting_value() ?? '0'));
            text.inputEl.setAttribute('type', 'number');
            text.onChange((value) => {
              const num_value = Number(value);
              if (!isNaN(num_value)) {
                set_setting_value(num_value);
              }
              if (typeof setting_config.callback === 'function') {
        ...
        case 'dropdown':
          setting.addDropdown(async (dropdown) => {
            ...
            dropdown.setValue(get_setting_value() || '');
        ...
        case 'slider':
          setting.addSlider((slider) => {
            ...
            slider.setValue(get_setting_value() || min);
            // slider.showTooltip();
